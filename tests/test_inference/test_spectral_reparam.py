...
# Reference values copied from the EOS class (sigma_scale = 1.5)
REPARAM_MEAN = np.array([0.92232738, 0.34177628, -0.08307007, 0.00413816])
REPARAM_L_WIDE = np.array(
    [
...
class TestSpectralReparamTransform:
    ...
    def test_tilde_recovers_known_gamma(self):
        """Given z, the recovered gamma = mu + L_wide @ z should match manual calc."""
        # Pick a simple z vector
        z = np.array([1.0, -1.0, 0.5, -0.5])
        expected_gamma = REPARAM_MEAN + REPARAM_L_WIDE @ z
