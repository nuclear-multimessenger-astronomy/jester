...
from .cosmology import convert_to_log10_mej_dyn
...
class MultiMessengerLikelihood(LikelihoodBase):
    ...
    and mass distribution from an EM-bright BNS merger. 
    The basic mechanism is as follows:
    
    1. Sample (m1, m2, d_L, cos_theta_jn) from the GW posterior.
    2. Interpolate tidal deformabilities (Λ1, Λ2) from the sampled EOS.
    3. Evaluate the conditional probabilities p(Λ1, Λ2 | m1, m2, d_L, cos_theta_jn, GW)
       through a conditional normalizing flow.
    4. Evaluate the mass distribution function on (m1, m2)
    5. Optional: If asked for, one can additional factor in a light curve posterior
       by evaluating the flow p(log10_mej_dyn, dL, cos_theta_jn | EM),
       similar to the MultiMessengerLikelihood. 
    6. Multiply and sum everything together to get the result.
    ...
    posterior_gw : str
        ...
        Has to include entries for mass_1_source, mass_2_source, luminosity_distance, cos_theta_jn.
    conditional_flow_gw: str
        Path to a conditional normalizing flow to evaluate p(Λ1, Λ2 | m1, m2, d_L, cos_theta_jn, GW)
    ...
    def setup_gw_likelihood(
        ...
        Nsamp = posterior_gw["mass_1_source"].shape[0]
        ...
        self.mass_1_source = posterior_gw["mass_1_source"][mask]
        self.mass_2_source= posterior_gw["mass_2_source"][mask]
        self.luminosity_distance = posterior_gw["luminosity_distance"][mask]
    ...
    def process_gw_likelihood(
        ...
        def process_single(sample):
            ...
            cond_sample = jnp.array([sample["mass_1_source"], sample["mass_2_source"], sample["luminosity_distance"], sample["cos_theta_jn"]])
            eval_sample = jnp.array([sample["lambda_1"], sample["lambda_2"]])
    ...
    def evaluate(self, params: dict[str, Float | Array]) -> Float:
        ...
        params : dict[str, Float | Array]
            ...
            - population parameters for self.mass_distribution_logpdf
        Returns
        -------
        Float
