...
- `type: "smc-partial-posteriors-rw"` - SMC on the "path of partial posteriors" (IBIS; data tempering, not lambda tempering)
  ...
  - **The compute graph only ever covers events assimilated so far, not every configured event.** `data_mask` is a runtime-traced array, so XLA cannot fold away a not-yet-assimilated event's (masked-to-0) flow evaluation -- it still gets computed, every substep. `sample()` tracks `active_names` (the already-assimilated prefix, grown by one group per batch) and rebuilds `ordered_event_vals`/`partial_logposterior_factory`/the jitted substep function via `_build_ordered_event_vals_fn(active_names)` **once per data-tempering batch boundary** (not once for the whole run, and not once per substep -- that substep-level recompilation trap is the different, already-fixed issue the paragraph above describes). A `StackedGWLikelihood` source contributing only some of a batch's active events is narrowed with `StackedGWLikelihood.subset()` (`likelihoods/gw.py`), which slices the already-loaded, already-stacked flow weights along their leading event axis -- no flow is reloaded or retrained. `_predict_full_jump_ess`'s auto-cadence look-ahead (below) uses the same helper, restricted to just the pending candidate events, for the same reason. `state.data_mask`'s array length grows with `active_names` across batches (it no longer stays fixed at `n_events`); it is resized (extended with zeros for the new group) before the first substep of a new batch, since the substep function reads it to build the resample-move target and that must line up with the batch's (larger) `ordered_event_vals`
