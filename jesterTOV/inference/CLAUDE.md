...
- `type: "smc-partial-posteriors-rw"` - SMC on the "path of partial posteriors" (IBIS; data tempering, not lambda tempering)
  ...
  - `plot_diagnostics()` produces the overall across-steps plot plus one per-step sub-step diagnostics plot (mask fraction/ESS/acceptance vs. sub-step) under `outdir/substep_diagnostics/`; `metadata["event_groups"]` lists the event names in each processed group. Logging and filenames refer to each group by a short `batch_<NN>` key (`NN` = absolute index of the group's first event in `event_order`) rather than the full `"+"`-joined event-name string — with many events, that string can get arbitrarily long and blow past OS filename limits (`OSError: File name too long`). The full `"+"`-joined name list is still logged once, when a batch starts, so the mapping from `batch_<NN>` to its constituent events is recoverable from the log
