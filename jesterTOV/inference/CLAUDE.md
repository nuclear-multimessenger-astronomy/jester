...
- `type: "smc-partial-posteriors-rw"` - SMC on the "path of partial posteriors" (IBIS; data tempering, not lambda tempering)
  ...
  - The MCMC move in each sub-step is built to target the *old* (pre-substep) mask rather than the new one -- the "resample-move" construction (Gilks & Berzuini 2001), also used by blackjax's own `smc.tempered.build_kernel` and by Chopin's `particles` reference implementation -- which makes the per-substep log-evidence increment exact at any mask-jump size, not just for small jumps; see `_build_jitted_substep_fn`'s docstring in `samplers/blackjax/smc/partial_posteriors.py`. Each event is still ramped in over an adaptive, uncapped ESS-targeting bisection (reusing `smc-rw`'s `target_ess` and blackjax's `dichotomy` root solver), now purely for MCMC-mixing/proposal-adaptation quality rather than evidence correctness. The bisection itself runs through `_build_jitted_ess_check_fn` (below), not a bare `blackjax.smc.ess.ess_solver` call -- a bare call rebuilds an unjitted closure and retraces/recompiles on every sub-step of every run (measured ~250x slower than pre-jitted in isolation)
  - Config is split into two levels: the top level (`event_order`, `warm_start_from`, `cadence`, `skip_move_when_ess_ok`) only orchestrates event assimilation; the nested `inner` block (`InnerSMCRandomWalkConfig`: `n_particles`, `n_mcmc_steps`, `target_ess`, `random_walk_sigma`) fully specifies the adaptive SMC-RW loop used to ramp in each event group
  ...
  - `_build_jitted_ess_check_fn` (`partial_posteriors.py`) is a single `jax.jit`-compiled, fixed-`(n_events,)`-shape ESS lookahead built once per run and shared by both the per-sub-step bisection above and the `cadence="auto"` full-jump predictor -- candidate event subsets are represented as a fixed-length 0/1 mask multiplied against the already-fixed-length per-event log-likelihood array, not a variable-length index gather, so this compiles exactly once regardless of group/queue size (a variable-length-gather version, the pre-optimization shape of both call sites, was measured to retrace/recompile per distinct queue length -- see PLAN.md in the repo root, "ESS-gated skip of resample+move", Phase 0 Finding 1 and the "Avoid shape-driven retracing" design note)
  - `skip_move_when_ess_ok` (`bool`, default `False`) opts into an ESS-gated skip of the resample+move step (Chopin's IBIS Algorithm 17.2), at the sub-event/fractional-mask granularity: before each sub-step, `_build_jitted_ess_check_fn` cheaply checks (no resample, no MCMC) whether the *entire* remaining jump for the current event group would keep ESS at or above `target_ess`; if so, `_build_jitted_reweight_only_step_fn` updates only the importance weights (ordinary un-resampled importance sampling, unbiased for the same reason a single IS step always is) and the resample+move is deferred -- potentially across several events, since the outer group loop simply continues once a group's mask reaches 1.0 via skips alone, extending the deferred span into the next group/event with no special-casing needed. `state.data_mask` is kept in sync with whatever `state.weights` currently represent at every step (real move or skip), which is exactly the invariant `_build_jitted_substep_fn`'s resample-move construction already depends on -- so when a real move eventually fires (because the next candidate jump would drop ESS below target), it calls the *same* `jitted_substep_fn` unmodified, transparently reweighting across the whole deferred span in one exact resample-move step. Per-sub-step diagnostics (`substep_diagnostics[batch_key]["skipped_history"]`, one bool per sub-step) record which sub-steps took the cheap branch; `_plot_substep_diagnostics` marks them with a red 'x' on the mask-fraction panel. A data-tempering step that finishes via skips alone has no real move at all that step, so its `acceptance_history` entry is `NaN` (not `0.0`) and `_build_metadata`'s `mean_acceptance` uses `jnp.nanmean` to ignore it. Defaults to `False` (today's behavior, byte-for-byte); validated in `debug/optimizing_pp_smc/ess_skip_toy_validation.py` (parent development workspace) against a closed-form conjugate-Gaussian toy model before relying on it in production -- see PLAN.md for the full design and validation results
  - Auto-cadence only: `metadata["auto_cadence_ess_history"]`/`metadata["auto_cadence_triggered_history"]` (also persisted under `histories/` in the saved `InferenceResult`) record, one entry per not-yet-assimilated event considered by `_next_auto_group`, the full-jump ESS computed after adding it to the pending queue and whether that check triggered the queue to be assimilated (`ess < auto_ess_threshold`) vs. left it queued. `plot_diagnostics()` turns this into `auto_cadence_ess.png`: one-shot ESS vs. "event added to queue" index, a dashed line at `auto_ess_threshold`, blue dots for deferred (queued) checks and red downward triangles where an update was triggered
  ...
  - `save_intermediate_results` (`bool`, default `True`) saves a full `InferenceResult` HDF5 (posterior + derived EOS quantities from the TOV solver, via `add_eos_from_transform`) after every data-tempering step, to `outdir/substep_results/results_<batch_key>.h5`, where `<batch_key>` is the same sequential `batch_<NN>` key used for logging and `substep_diagnostics/<batch_key>.png` (not the absolute event-index-based numbering used before) — stable/collision-free across warm-started runs since `batch_key` already accounts for `n_prev_batches`. Lets users inspect how the posterior evolves batch-by-batch (e.g. which events are most informative). Implementation: `_save_intermediate_result` temporarily swaps `self._particles_flat`/`self._weights`/`self.final_state`/`self.metadata` to a "so far" snapshot (built by `_build_metadata`, also reused for the real end-of-run metadata) so it can drive `InferenceResult.from_sampler` through the normal `get_sampler_output()` path, then restores them in a `finally` block. Needs the full `InferenceConfig` and outdir, which this sampler doesn't otherwise have — `run_inference.py` wires these in via `sampler.configure_intermediate_saving(...)` right after `create_sampler`, before `sample()` runs.
