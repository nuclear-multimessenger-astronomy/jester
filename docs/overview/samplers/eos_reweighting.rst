...
Postprocessing only produces the mass-radius and mass-Lambda plots, since the resampled posterior only contains :math:`(M, \Lambda, R)` curves — there is no NEP/CSE parameter posterior, density/pressure/cs2 profile, or TOV central-density diagnostic to build the other plots from. It runs automatically after ``run_jester_inference config.yaml`` and can also be triggered standalone with ``run_jester_postprocessing config.yaml``. To overlay a known injected EOS, point ``injection_eos_path`` at an NPZ file in the format expected by :func:`~jesterTOV.inference.postprocessing.postprocessing.load_injection_eos` — the tabulated LALSuite EOS tables shipped under ``jesterTOV/tabulated_eos/lalsuite/`` (e.g. ``MPA1.npz``, ``SLY230A.npz``) already follow this format and are convenient injections for validation runs.
