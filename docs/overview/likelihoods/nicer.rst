...
.. plot:: overview/likelihoods/nicer_mr_plot.py
   ...
   where both the Amsterdam and Maryland groups have published an independent
   analysis, both are overlaid using the preferred/headline dataset from each
   group, with solid contours for Amsterdam and dashed contours for Maryland.
...
Pretrained flows are shipped with the ``jester`` repository along with the posterior samples downloaded from Zenodo releases.
The flows are stored in the directory ``jesterTOV/inference/flows/models/nicer_maf/``.
...
If multiple analysis groups are used (e.g. Amsterdam and Maryland for PSR J0030+0451), the final log-likelihood averages over groups (with equal weights) in the same way as the above equation.

Mass samples that exceed :math:`M_\mathrm{TOV}` receive a large negative penalty, if enabled by the user (switched off by default).
...
Mass-radius posterior samples for all supported pulsars are stored as compressed NumPy archives (``.npz``) under ``jesterTOV/inference/data/NICER/``. 
Each file contains ``radius`` (km), ``mass`` (solar masses), and a ``metadata`` dictionary with the pulsar name, analysis group, hotspot model, Zenodo record URL, and paper reference.

The raw Zenodo archives (which can be several gigabytes) are downloaded into ``jesterTOV/inference/data/NICER/zenodo_data/`` (gitignored). 
Only the lightweight extracted ``.npz`` files are version-controlled.
...
Two hotspot geometries x two prior choices:
...
**Amsterdam group — Choudhury et al. 2024** (`ApJL 971, L20 <https://inspirehep.net/literature/2806113>`_; `Zenodo 13766753 <https://zenodo.org/records/13766753>`_)

Headline result using CST+PDT hotspot model with 3C50 background:

* ``J04374715_amsterdam_CST_PDT_NICER_only_Choudhury2024.npz``

**Maryland group — Miller, Dittmann, Holt et al. 2026** (`ApJL 1000, L48 <https://inspirehep.net/literature/3091115>`_; `Zenodo 17833896 <https://zenodo.org/records/17833896>`_)
...
**Amsterdam group — Salmi et al. 2024** (`ApJ 974, 294 <https://inspirehep.net/literature/2800506>`_; `Zenodo 10519473 <https://zenodo.org/records/10519473>`_)
