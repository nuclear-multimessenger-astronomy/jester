...
eos:
  ...
  crust_name: "DH"     # Crust model (any file in jesterTOV/crust_files, e.g. "DH", "BPS", "SLy")
...
- **`crust_name`** (`str`, default: `"DH"`) - Crust model name, matching any `.npz` file in `jesterTOV/crust_files` (e.g. `"DH"`, `"BPS"`, `"SLy"`)
...
eos:
  ...
  crust_name: "SLy"        # Crust model (any file in jesterTOV/crust_files; recommended "SLy" for LALSuite compatibility)
...
- **`crust_name`** (`str`, default: `"DH"`) - Crust model name, matching any `.npz` file in `jesterTOV/crust_files` (e.g. `"DH"`, `"BPS"`, `"SLy"`). Use `"SLy"` for LALSuite compatibility.
...
sampler:
  ...
  cadence: 1                          # Events per data-tempering step; int, list[int], or "auto" (see below)
  auto_ess_threshold: null            # Full-jump ESS trigger threshold; only used when cadence: "auto"
  skip_move_when_ess_ok: false        # Opt-in ESS-gated skip of resample+move (see below)
  save_intermediate_results: true     # Save a full InferenceResult HDF5 after each data-tempering step
...
- **`warm_start_from`** (`str | None`, default: `None`) - Path to a previous run's `InferenceResult` HDF5 file. When set, the initial particles are resampled (with replacement, uniform weights) from that run's posterior instead of the prior. The source run does not have to be a partial-posteriors run — any sampler works, since the "already covered" GW events are derived from the previous run's *saved config* (`likelihoods` section), not from sampler-specific metadata. That derived event list must be a strict prefix of this run's `event_order` — the already-covered events are not replayed (their mask entries start, and stay, at 1); only the newly added event(s) are assimilated. The source run may have had *no* GW events at all (e.g. a radio- or ChiEFT-only run): the empty list is trivially a strict prefix, so every configured GW event is assimilated from scratch, with only the initial particles coming from that run's posterior. The previous run's always-on (non-GW) likelihoods must also exactly match this run's (same types and kwargs, e.g. the same ChiEFT data path); a mismatch raises a `ValueError` since the cumulative logZ bookkeeping depends on that part being unchanged. This is the sequential N → N+1 use case: warm-start a run adding one more GW event from a previous run's converged posterior (which can itself be a plain `smc-rw` run, or a GW-free run whose posterior is only used to seed initial particle positions).
- **`cadence`** (`int | list[int] | "auto"`, default: `1`) - Controls how many of the *new* events (the ones not already covered by `warm_start_from`) are turned on together per data-tempering step, instead of the default one event at a time. All events in a group share a single mask fraction that is bisected from 0 to 1 jointly (the same ESS-targeting machinery applies unmodified, since the group's combined log-likelihood is still linear in the shared fraction). An `int` chunks the new events into fixed-size groups in order, with the final group taking the remainder if the count doesn't divide evenly (e.g. `cadence: 5` over 12 new events gives groups of 5, 5, 2). A `list[int]` gives explicit group sizes, e.g. `cadence: [10, 20, 30, 50]` to process 100 new events as four steps of that size — the list must sum to exactly the number of new events for this run, checked at sample time (once `warm_start_from` is resolved) rather than at config-parse time, since it depends on how many events the warm start already covers.
  - `cadence: "auto"` (or `"automatic"`) builds groups dynamically instead of following a fixed schedule. Not-yet-assimilated events are added to a pending queue one at a time; after each addition, a *full-jump* effective sample size is computed by reweighting the current particle cloud with the queue's combined log-likelihood alone — no annealing, no MCMC move, just a read-only importance-weighting check (the same diagnostic the sub-step bisection already evaluates internally as its very first probe, see `_build_jitted_ess_check_fn` in `partial_posteriors.py`). While that ESS stays at or above `auto_ess_threshold`, the newest event is judged "not surprising" and stays queued instead of triggering an update; once it drops below the threshold (or the last configured event is reached), the whole queue is assimilated together in one data-tempering step, through the same unbiased sub-step bisection used for a fixed-size group. Use this to automate the cadence itself: batches grow while incoming events agree with the current posterior, and shrink (down to a single event) when something informative or in tension arrives.
- **`auto_ess_threshold`** (`float | None`, default: `None`) - Full-jump ESS threshold in `(0, 1]` used only when `cadence: "auto"`. `None` falls back to `inner.target_ess` — the same value the sub-step bisection itself targets, which is also the most conservative choice (it only defers events the bisection would have taken in a single free sub-step anyway). A looser threshold (e.g. 0.5-0.7) defers more aggressively, batching more events together before triggering an update. Setting this field with any `cadence` other than `"auto"` raises a validation error at config-parse time, since it would otherwise silently have no effect.
- **`skip_move_when_ess_ok`** (`bool`, default: `False`) - Opt-in ESS-gated skip of the resample+move step (Chopin's IBIS Algorithm 17.2), at the sub-event/fractional-mask granularity, rather than only at the whole-group granularity `cadence: "auto"` already offers. Before each sub-step, a cheap, move-free check asks whether the *entire* remaining jump for the current event group would keep ESS at or above `target_ess`; if so, only the importance weights are updated (ordinary un-resampled importance sampling, unbiased for the same reason a single importance-sampling step always is) and the resample+move is deferred — potentially across several events, since deferral naturally continues into the next event group once the current one's mask reaches 1.0 via skips alone. A real move fires as soon as a candidate jump would actually drop ESS below target, reweighting across the whole deferred span in one exact step — the per-substep log-evidence increment stays exact at any jump size regardless of how many sub-steps were skipped beforehand (see the sampler class's module docstring in `partial_posteriors.py`). Defaults to `False` (today's behavior, byte-for-byte unchanged); this changes the sampling trajectory (though not the correctness of the evidence/posterior estimators), so validate it against a specific run before relying on it in production. `metadata["skip_move_when_ess_ok"]` records which behavior produced a saved result, and per-substep diagnostics record which sub-steps took the cheap branch (visualized as red 'x' markers on the mask-fraction panel of the sub-step diagnostics plot).
- **`save_intermediate_results`** (`bool`, default: `True`) - Save a full `InferenceResult` HDF5 (posterior samples, derived EOS quantities from the TOV solver, and metadata) after each data-tempering step, to `outdir/substep_results/results_<n>.h5` where `<n>` is the 1-indexed position of the step's first event within the full configured `event_order` (so numbering stays consistent across warm-started runs). This lets you inspect how the posterior evolves as events are assimilated one batch at a time — e.g. to see which events are most informative — using the same `InferenceResult.load(...)` API as the final result. Set to `False` to skip the extra TOV solve (over `n_eos_samples` samples, same as the final result) per step, for runs that don't need the intermediate posteriors.
- **`inner`** (`InnerSMCRandomWalkConfig`) - Configuration of the adaptive SMC-RW loop used to ramp in each event's mask fraction from 0 to 1. Each sub-step's MCMC move targets the *old*, pre-substep mask rather than the new one (the "resample-move" construction of Gilks & Berzuini 2001, also used by blackjax's own `smc.tempered` and by Chopin's reference `particles` package), which makes the per-substep log-evidence increment exact at any mask-jump size — a move that leaves the old target invariant cannot change the particles' marginal distribution, so reweighting the post-move particles by the plain incremental likelihood ratio is unbiased regardless of step size. Events are nonetheless still ramped in over several small fractional mask increments via an ESS-targeting bisection search (`inner.target_ess`) — the same machinery `smc-rw`'s adaptive $\lambda$ schedule uses, applied to the mask fraction instead of $\lambda$ — now purely for MCMC-mixing/proposal-adaptation quality rather than evidence correctness. This works because the mask-weighted logposterior is linear in the fraction of the event currently being ramped in. The number of sub-steps is uncapped: the search keeps taking ESS-targeting increments until the mask reaches 1.0.
...
- Per-step (one or more events, per `cadence`) effective sample size (ESS), acceptance rate, number of sub-steps taken, and cumulative log evidence (instead of per-tempering-step) — when warm-starting, these histories cover only the newly assimilated event(s), not the replayed prefix
- One SMC-diagnostics-style plot per data-tempering step (mask fraction / ESS / acceptance vs. sub-step) under `outdir/substep_diagnostics/`, plus the overall across-steps plot (`smc_diagnostics.png`)
- Unless `save_intermediate_results: false`: one full `InferenceResult` HDF5 per data-tempering step under `outdir/substep_results/`
